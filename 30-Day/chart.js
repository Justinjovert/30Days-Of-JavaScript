...
const fetchPop = async () => {
    try {
        ...
        for (const country of countries) {
            populationList.set(country.name.common, country.population)
        }
        ...
        sortedPopList.unshift(['World', totalPopulation])
...
const fetchLang = async () => {
    try {
        ...
        //Create a map
        //Add language as a key with a value of 1
        //If it already exists, add value by 1
        const languageList = new Map()
        for (const country of countries) {
            if (country.languages) {
                const thisLanguageArray = Object.values(country.languages)
                thisLanguageArray.forEach(thisLanguage => {
                    if (languageList.has(thisLanguage)) {
                        languageList.set(thisLanguage, languageList.get(thisLanguage) + 1)
                    }
                    else {
                        languageList.set(thisLanguage, 1)
                    }
                });
...
const dataCharts = (data) => {
    ...
    for (let ctr = 0; ctr < data.length; ctr++) {
        ...
        const formattedNumber = Number(data[ctr]).toLocaleString('en-US')
...
const populationTab = document.getElementById('populationTab')
const languageTab = document.getElementById('languagesTab')




//Create Chart but function
/* const createChart = (sortedPopList) => {
    const phraseLabel = 'Population of Countries'
    console.log(sortedPopList)
    if (popLabelsArr == null) {
        sortedPopList.forEach(country => {

            console.log(country[0])
            const labels = country.map(([name]) => name);
            popLabelsArr = Array.from(labels)
            const data = country.map(([, count]) => count);
            popDataArr = Array.from(data)
            popLabelsArr = country[0]
            popDataArr = country[1]
...
tabPop.addEventListener('click', e => {
    ...
    popLabelsArr = []
    popDataArr = []
    searchBarSorted.slice(0, 10).forEach(element => {
        const [populationLabel, populationData] = element
        popLabelsArr.push(populationLabel)
        popDataArr.push(populationData)
    })
    dataLabels(popLabelsArr)
    initChart(popChart, popLabelsArr, popDataArr, phraseLabel)
    dataCharts(popDataArr)
